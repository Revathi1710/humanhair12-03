...
const Allproducts = () => {
  ...
  axios.post(`${process.env.REACT_APP_API_URL}/getVendorProductcount`, { vendorId })
  .then(response => {
    if (response.data.status === 'ok') {
...
{product.approved ? (
    <span className='approved-items'>Approved</span>
) : (
    <span className='reject-items'>Reject</span>
)}
